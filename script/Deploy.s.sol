...
/// @notice Deploys the M1 skeleton contract with the supplied USDC address, admin multisig, and
...
contract Deploy is Script {
    function run() external returns (Settlement settlement) {
        ...
        vm.startBroadcast();
        settlement = new Settlement(IERC20(usdc), admin, TimeWindows({tw1: tw1, tw2: tw2, tw3: tw3}));
        vm.stopBroadcast();
    }
}
